...
void main() async {
  ...
  // Открытие шифрованного бокса для избранного
  ...
  // Генерация второго ключа для демонстрации ошибки чтения
...
class _HomePageState extends State<HomePage> {
  ...
  // Метод для демонстрации неправильного ключа
  void _demonstrateWrongKey() async {
    // Создаем временный бокс с неправильным ключом
    final wrongKey = utf8.encode('wrongKey456');
    final wrongEncryptionKey = sha256.convert(wrongKey).bytes;

    try {
      // Пытаемся открыть бокс с неправильным ключом
      final wrongBox = await Hive.openBox<FavoriteItem>(
        'wrongKeyDemo',
        encryptionCipher: HiveAesCipher(wrongEncryptionKey),
      );

      // Пытаемся получить данные из бокса favorites через неправильный ключ
      final favoritesBox = Hive.box<FavoriteItem>('favorites');
      final data = favoritesBox.values.toList();

      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('Wrong Key Test'),
          content: const Text('You should not see this dialog. If you do, encryption is not working correctly.'),
          actions: [
            TextButton(
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: const Text('Wrong Key Test'),
          content: Text('Error reading with wrong key: $e\n\nThis is expected behavior.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: const Text('Close'),
  ...
  // Метод для сжатия данных в боксе
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.person_switch),
...
// Экран для добавления и редактирования продуктов
